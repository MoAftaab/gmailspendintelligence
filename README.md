...
- **Priority-based AI selection and batching** — after the deterministic parser evaluates the full scanned set, every `UNCERTAIN` email is sent to the LLM in sequential batches; there is no top-10 cutoff. `OPENAI_MAX_EMAILS` controls the maximum emails per request, so N uncertain emails produce `ceil(N / OPENAI_MAX_EMAILS)` requests. Clear validated transactions remain deterministic, and promotional messages are excluded from the LLM batches. The Gmail scan scope itself is controlled separately by `GMAIL_MAX_MESSAGES`.
